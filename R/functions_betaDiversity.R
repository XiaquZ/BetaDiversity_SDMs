...
# ---------------------------------------------------------
list_metric_tiles <- function(out_dir, pattern) {
  message("Searching for files in: ", out_dir)
...
merge_tile_outputs <- function(files,
  ...
  message("Target output: ", out_file)

  files <- unlist(files, use.names = FALSE)
  message("Number of input paths supplied: ", length(files))
  files <- files[file.exists(files)]
  message("Number of existing files to merge: ", length(files))

  if (length(files) == 0) {
    stop("No existing files supplied to merge_tile_outputs().")
  }

  dir.create(dirname(out_file), recursive = TRUE, showWarnings = FALSE)

  # Build a virtual mosaic first
  message("Building VRT from ", length(files), " tiles ...")
  v <- terra::vrt(files)

  message("VRT created successfully.")
  message("Writing merged raster to disk ...")

  if (is.null(wopt)) {
    terra::writeRaster(v, out_file, overwrite = overwrite)
  } else {
    terra::writeRaster(v, out_file, overwrite = overwrite, wopt = wopt)
  }

  message("Finished merge: ", out_file)
  message("--------------------------------------------------")
